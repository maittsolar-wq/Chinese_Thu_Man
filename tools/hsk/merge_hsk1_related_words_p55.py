"""P5.5 -- HSK1 Related-Word Production Integration merge script.

Scope: merge `relatedWordIds` from the approved, P5.4.4-signed-off HSK1
related-word refined-selection artifact into HSK1 production vocabulary,
for `status == "selected"` records ONLY. `needs_review` records (8 of
them) receive NO relatedWordIds in this phase -- their record is left
byte-for-byte as-is. This mirrors the established HSK6 P5.3 integration
pattern (tools/hsk/merge_hsk6_related_words_p53.py) as closely as the
two source schemas allow.
...
Source artifact (READ-ONLY, never written by this script):
    ...
    "selectedCount", "selectionReasons"} -- NOTE the source field is
    named `selectedRelatedWordIds`, renamed to `relatedWordIds` on the
    production record to match the production/app schema, exactly as
    the HSK6 merge did.)

Production target (the ONLY file this script may write):
    data/hsk/hsk1/hsk1_vocabulary_production.json

Schema note vs. HSK6: every HSK1 production record already carries a
`relatedWordIds` key (as `[]`), unlike HSK6 where the key was absent
pre-merge. An existing value of `[]` (or a missing key) is treated as
"not yet integrated" and may be written; any existing NON-EMPTY value
is treated as a real conflict and aborts the run (fail-closed, same as
HSK6).
...
The merge contract is intentionally narrow:
  ...
  - No other field on any production record is read, touched, or
    reordered. All 1,500 HSK2-6 production records and all HSK1 records
...
Usage:
    python merge_hsk1_related_words_p55.py --dry-run
    python merge_hsk1_related_words_p55.py --apply
"""
...
EXPECTED_SELECTED_RELATIONSHIPS = 366  # relationships within status=="selected" records only
...
def load_json_text(path: Path) -> str:
    # Source/production files may carry CRLF line endings; read in
    # binary-safe text mode without translating line endings so we can
    # reproduce the exact byte layout on write.
...
def validate_source(source_doc: dict) -> dict:
    ...
    if len(records) != EXPECTED_SOURCE_RECORDS:
        fail(
            f"source record count {len(records)} != expected "
...
def compute_plan(production_records: list, selected: list) -> dict:
    ...
    value. An existing value of [] (or a missing key) counts as "not
    yet integrated". Never mutates the input lists."""
    ...
    for src_r in selected:
        ...
        if existing_value == new_value:
            ...
            # existing is [] (or key absent) -- not yet integrated, safe to add.
            to_add.append((prod_r, new_value))
        else:
            conflicts.append(
                {"id": target_id, "existing": existing_value, "incoming": new_value}
            )
...
def serialize_like_source(records: list) -> str:
    ...
    already used by hsk1_vocabulary_production.json: 2-space indent,
